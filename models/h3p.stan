...
transformed parameters {
  ...
  matrix[4, N] pos_pm = [ra, dec, pmra, pmdec]; // mas/yr for the pm part
  ...
  profile("observation") {
    ...
    for (i in 1 : N) {
      if (pos_obs[i] && pm_obs[i]) {
        ...
        if (pos_obs[i]) {
          pos_measured[i] ~ multi_normal_cholesky(pos_pm[1 : 2, i],
                                                  pos_cov_mats[i]);
        }
        if (pm_obs[i]) {
          pm_measured[i] ~ multi_normal_cholesky(pos_pm[3 : 4, i],
                                                 pm_cov_mats[i]);
        }
      }
      if (dist_obs[i]) {
